...
if ($_REQUEST[pagina]=='') 
{
   ...
   $pagina = $_REQUEST['pagina'];
	...
	$consultar=mysql_query("SELECT * FROM vparticipantes Where idcomit='$consulta[idcomit]' and anio=2016", $conexion); //$_GET['catcod'] = variable a ser pasada por URL
	$total_registros = mysql_num_rows($consultar);
	$total_paginas = ceil($total_registros / $registros);
  //AQUI EL FILTRO DE DATOS	
	$consultar = mysql_query("SELECT * FROM vparticipantes Where idcomit='$consulta[idcomit]' and anio=2016 ORDER BY idpar ASC LIMIT $inicio, $registros");
